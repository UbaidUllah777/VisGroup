import React from "react";
import icon1 from "../../assets/images/icon/icon_61.svg";
import icon2 from "../../assets/images/icon/icon_62.svg";
import icon3 from "../../assets/images/icon/icon_63.svg";
import { Link } from "react-router-dom";

export const serviceContent = [
  // ---------------- TeleCommunication-----------------
  {
    id: "teleCommunication",
    title: "TeleCommunication",
    link: "",
    areaServices: [
      // Networkbenchmark Testing
      {
        id: "networkbenchmarkTesting",
        icon: icon1,
        delayAnimation: 0,
        link: "/services/telecommunication",
        title: "Networkbenchmark Testing",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris.
          </>
        ),
      },

      // Field Performance Testing
      {
        id: "fieldPerformanceTesting",
        icon: icon1,
        delayAnimation: 0,
        link: "/services/telecommunication",
        title: "Field Performance Testing",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris. high-rated mobile communications.
          </>
        ),
      },

      // Global Certification Forum
      {
        id: "globalCertificationForum",
        icon: icon1,
        delayAnimation: 0,
        link: "/services/telecommunication",
        title: "Global Certification Forum",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris. high-rated mobile communications.
          </>
        ),
      },

      // SoS Calls Testing
      {
        id: "SoSCallsTesting",
        icon: icon1,
        delayAnimation: 0,
        link: "/services/telecommunication",
        title: "SoS Calls Testing",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris. high-rated mobile communications.
          </>
        ),
      },
    ],
  },

  //---------------- Infotainment Systems -----------
  {
    id: "infotainmentSystems",
    title: "Infotainment Systems",
    link: "",
    areaServices: [
      // GPS Testing
      {
        id: "gpsTesting",
        icon: icon2,
        delayAnimation: 100,
        link: "/services/infotainmentsystems",
        title: "GPS Testing ",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris.
          </>
        ),
      },

      //Tuner And Media Testing
      {
        id: "tunerAndMediaTesting",
        icon: icon2,
        delayAnimation: 100,
        link: "/services/infotainmentsystems",
        title: "Tuner And Media Testing",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris.by famous automotive brands
          </>
        ),
      },

      //HMI Testing
      {
        id: "hmiTesting",
        icon: icon2,
        delayAnimation: 100,
        link: "/services/infotainmentsystems",
        title: "HMI Testing",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris.by famous automotive brands
          </>
        ),
      },
    ],
  },

  // -----------Software Development------------------

  {
    id: "softwareDevelopment",
    title: "Software Development",
    link: "",
    areaServices: [
      // Web Apps Development
      {
        id: "webAppsDevelopment",
        icon: icon3,
        delayAnimation: 200,
        link: "/services/softwaredevelopment",
        title: "Web Apps Development",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris.end-to-end solutions that match business
            requirements.
          </>
        ),
      },

      // Mobile Apps Development
      {
        id: " mobileAppsDevelopment",
        icon: icon3,
        delayAnimation: 200,
        link: "/services/softwaretesting",
        title: " Mobile Apps Development",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris.end-to-end solutions that match business
            requirements.
          </>
        ),
      },
    ],
  },

  // -----------Quality Assurance------------------

  {
    id: "Quality Assurance",
    title: "Quality Assurance",
    link: "",
    areaServices: [
      // Manual Testing
      {
        id: "manualTesting",
        icon: icon3,
        delayAnimation: 200,
        link: "/services/softwaretesting",
        title: "Manual Testing",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris.end-to-end solutions that match business
            requirements.
          </>
        ),
      },

      // Automation Testing
      {
        id: "automationTesting",
        icon: icon3,
        delayAnimation: 200,
        link: "/services/softwaretesting",
        title: "Automation Testing",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris.end-to-end solutions that match business
            requirements.
          </>
        ),
      },

      // Performance Testing
      {
        id: "performanceTesting",
        icon: icon3,
        delayAnimation: 200,
        link: "/services/softwaretesting",
        title: "Performance Testing",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris.end-to-end solutions that match business
            requirements.
          </>
        ),
      },

      // API Testing
      {
        id: "apiTesting",
        icon: icon3,
        delayAnimation: 200,
        link: "/services/softwaretesting",
        title: "API Testing",
        text: (
          <>
            Ut adipisicing irure sint id deserunt sit quis dolor anim magna
            cupidatat. Ullamco sint occaecat id in exercitation quis laboris
            consequat. Ea occaecat est esse consequat irure deserunt sint quis.
            Aliqua nulla labore Lorem ex Lorem velit tempor minim labore
            do.Labore labore dolore occaecat anim nulla culpa non voluptate
            irure quis. Dolor est ex nulla nostrud exercitation tempor ipsum
            ullamco voluptate. Minim quis mollit velit deserunt magna elit
            excepteur. Consequat labore tempor labore enim. Eu ipsum ex
            incididunt dolore Lorem laboris dolor anim veniam aute incididunt
            occaecat laboris.end-to-end solutions that match business
            requirements.
          </>
        ),
      },
    ],
  },
];
const MainFourServices = () => {
  const firstFourServices = serviceContent.slice(0, 4);
  return (
    <>
      {firstFourServices.map((item) => (
        <div
          className="col-md-6 col-sm-12 d-flex mt-45"
          data-aos="fade-up"
          data-aos-delay={item.delayAnimation}
          key={item.id}
        >
          <div className="card-style-seven">
            <div className="icon-title-container" style={{ display: "flex" }}>
              <div
                className="icon d-flex align-items-center justify-content-center"
                style={{ marginRight: "10px" }}
              >
                <img src={item.icon} alt="icon" />
              </div>
              <h4 style={{ marginTop: "12px" }}>{item.title}</h4>
            </div>
            <p>{item.text}</p>
            <Link type="button" className="theme-btn-three" to={item.link}>
              Read More..
            </Link>
          </div>
        </div>
      ))}
    </>
  );
};

export const AllServices = () => {
  return (
    <>
      {serviceContent.map((serviceGroup) =>
        serviceGroup.areaServices.map((item) => (
          <div
            className="col-md-4 col-sm-12 d-flex mt-45"
            data-aos="fade-up"
            data-aos-delay={item.delayAnimation}
            key={item.id}
          >
            <div className="card-style-seven">
              <div className="icon-title-container" style={{ display: "flex" }}>
                <div
                  className="icon d-flex align-items-center justify-content-center"
                  style={{ marginRight: "12px" }}
                >
                  <img src={item.icon} alt="icon" />
                </div>
                <h4 style={{ marginTop: "10px" }}>{item.title}</h4>
              </div>
              <p className="UpToFourLines">{item.text}</p>
              <Link type="button" className="theme-btn-three" to={item.link}>
                Read More..
              </Link>
            </div>
          </div>
        ))
      )}
    </>
  );
};

export default MainFourServices;
